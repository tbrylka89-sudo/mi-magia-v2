...
<head>
  ...
  <style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .secondary-content {
      background-color: rgba(184, 168, 138, 0.08);
      padding: 20px;
...
async function sendDay7Email({ customerName, customerEmail, guardianName, orderNumber, formToken }) {
  const title = "Día 7 — La canalización ha terminado";
  const confirmUrl = formToken
    ? `https://magia.duendesdeluruguay.com/api/confirm-delivery?token=${formToken}`
    : null;
  ...
  const content = `
    ...
    ${confirmUrl ? `
    <div style="text-align: center; margin: 30px 0; padding: 24px; background: rgba(139, 105, 20, 0.06); border-radius: 8px;">
      <p style="font-size: 15px; margin-bottom: 16px; color: ${colors.text};">Cuando ${guardianName} llegue a tus manos, confirmá que ya lo recibiste. A partir de ese momento, se activa la siguiente etapa de tu experiencia.</p>
      <a href="${confirmUrl}" style="display: inline-block; background-color: ${colors.button}; color: white; padding: 14px 32px; text-decoration: none; border-radius: 4px; font-weight: 600; font-family: sans-serif; letter-spacing: 0.5px;">Confirmar recepción</a>
    </div>
    ` : ""}
