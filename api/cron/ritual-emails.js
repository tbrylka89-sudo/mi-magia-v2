...
async function markEmailAsSent(client, orderId, emailType, resendEmailId) {
  await client.query(
    `INSERT INTO ritual_emails_sent (order_id, email_type, resend_email_id, sent_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (order_id, email_type) DO UPDATE SET resend_email_id = $3, sent_at = NOW()`,
    [orderId, emailType, resendEmailId]
  );
}

async function sendRitualEmail(day, orderData) {
  const { id: orderId, customer_name, customer_email, guardian_name, form_token } = orderData;
  ...
  try {
    ...
    // Pass formToken only for Day 7 (confirm delivery link)
    if (day === 7 && form_token) {
      params.formToken = form_token;
    }

    const result = await emailFunctions[day](params);
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const ordersResult = await client.query(
      `SELECT id, customer_name, customer_email, guardian_name, form_submitted_at, form_token
       FROM orders
       WHERE form_submitted_at >= $1
       AND ritual_email_status = 'active'
